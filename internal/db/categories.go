...
var CategoryWhere = struct {
	...
	UUID:      whereHelperstring{field: "\"categories\".\"uuid\""},
...
type categoryR struct {
	WorksCategories WorksCategorySlice `boil:"WorksCategories" json:"WorksCategories" toml:"WorksCategories" yaml:"WorksCategories"`
}
...
var (
	categoryAllColumns            = []string{"id", "uuid", "name", "created_at", "updated_at"}
	categoryColumnsWithoutDefault = []string{"uuid", "name", "created_at", "updated_at"}
...
// WorksCategories retrieves all the works_category's WorksCategories with an executor.
func (o *Category) WorksCategories(mods ...qm.QueryMod) worksCategoryQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"works_category\".\"category_id\"=?", o.ID),
	)

	query := WorksCategories(queryMods...)
	queries.SetFrom(query.Query, "\"works_category\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"works_category\".*"})
	}

	return query
}

// LoadWorksCategories allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (categoryL) LoadWorksCategories(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCategory interface{}, mods queries.Applicator) error {
	var slice []*Category
	var object *Category
	...
	query := NewQuery(
		qm.From(`works_category`),
		qm.WhereIn(`works_category.category_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load works_category")
	}

	var resultSlice []*WorksCategory
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice works_category")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on works_category")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for works_category")
	}
	...
	if singular {
		object.R.WorksCategories = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &worksCategoryR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.CategoryID {
				local.R.WorksCategories = append(local.R.WorksCategories, foreign)
				if foreign.R == nil {
					foreign.R = &worksCategoryR{}
				}
...
// AddWorksCategories adds the given related objects to the existing relationships
// of the category, optionally inserting them as new records.
// Appends related to o.R.WorksCategories.
// Sets related.R.Category appropriately.
func (o *Category) AddWorksCategories(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*WorksCategory) error {
	var err error
	...
	if o.R == nil {
		o.R = &categoryR{
			WorksCategories: related,
		}
	} else {
		o.R.WorksCategories = append(o.R.WorksCategories, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &worksCategoryR{
				Category: o,
			}
		} else {
			rel.R.Category = o
