...
var TagWhere = struct {
	...
	UUID:      whereHelperstring{field: "\"tags\".\"uuid\""},
...
var TagRels = struct {
	BacknumberTags string
}{
	BacknumberTags: "BacknumberTags",
}

// tagR is where relationships are stored.
type tagR struct {
	BacknumberTags BacknumberTagSlice `boil:"BacknumberTags" json:"BacknumberTags" toml:"BacknumberTags" yaml:"BacknumberTags"`
}
...
var (
	tagAllColumns            = []string{"id", "uuid", "name", "created_at", "updated_at"}
	tagColumnsWithoutDefault = []string{"uuid", "name", "created_at", "updated_at"}
...
// BacknumberTags retrieves all the backnumber_tag's BacknumberTags with an executor.
func (o *Tag) BacknumberTags(mods ...qm.QueryMod) backnumberTagQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"backnumber_tag\".\"tag_id\"=?", o.ID),
	)

	query := BacknumberTags(queryMods...)
	queries.SetFrom(query.Query, "\"backnumber_tag\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"backnumber_tag\".*"})
	}

	return query
}

// LoadBacknumberTags allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (tagL) LoadBacknumberTags(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTag interface{}, mods queries.Applicator) error {
	var slice []*Tag
	var object *Tag
	...
	query := NewQuery(
		qm.From(`backnumber_tag`),
		qm.WhereIn(`backnumber_tag.tag_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load backnumber_tag")
	}

	var resultSlice []*BacknumberTag
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice backnumber_tag")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on backnumber_tag")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for backnumber_tag")
	}
	...
	if singular {
		object.R.BacknumberTags = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &backnumberTagR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.TagID {
				local.R.BacknumberTags = append(local.R.BacknumberTags, foreign)
				if foreign.R == nil {
					foreign.R = &backnumberTagR{}
				}
				foreign.R.Tag = local
...
// AddBacknumberTags adds the given related objects to the existing relationships
// of the tag, optionally inserting them as new records.
// Appends related to o.R.BacknumberTags.
// Sets related.R.Tag appropriately.
func (o *Tag) AddBacknumberTags(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*BacknumberTag) error {
	var err error
	...
	if o.R == nil {
		o.R = &tagR{
			BacknumberTags: related,
		}
	} else {
		o.R.BacknumberTags = append(o.R.BacknumberTags, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &backnumberTagR{
				Tag: o,
			}
		} else {
			rel.R.Tag = o
...
func (o *Tag) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
		return errors.New("db: no tags provided for upsert")
	}
	if !boil.TimestampsAreSkipped(ctx) {
		currTime := time.Now().In(boil.GetLocation())
	...
	if !cached {
		...
		if updateOnConflict && len(update) == 0 {
			return errors.New("db: unable to upsert tags, could not build update column list")
		}
	...
	if err != nil {
		return errors.Wrap(err, "db: unable to upsert tags")
	}
